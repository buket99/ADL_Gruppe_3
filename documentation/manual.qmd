...
-   Git
-   Docker
-   An X server (XQuartz on macOS, WSL2 with VcXsrv or X410 on Windows)
-   OpenAI API Key
...
-   **Camera not found**: Ensure your webcam is properly connected and recognized by your operating system. If you're using Docker, make sure you've correctly passed the device to the container (e.g., `--device=/dev/video0:/dev/video0`).
-   **GUI not displaying**: If you're using Docker, ensure you've set up X11 forwarding correctly (e.g., `-v /tmp/.X11-unix:/tmp/.X11-unix:rw -e DISPLAY=$DISPLAY`). On macOS, install XQuartz. On Windows, use WSL2 with an X server like VcXsrv or X410.
